...
class Cel extends BitmapData {
   ...
   public function new(sprite:Aseprite, chunk:CelChunk) {
      ...
      var pixels:ByteArray = new ByteArray(chunk.width * chunk.height * 4);
...
class Color {
   ...
   public static function indexedToargb(sprite:Aseprite, index:Int):Null<UInt> {
      ...
      if (sprite.aPalette.entries.exists(index)) {
         return sprite.aPalette.entries.get(index);
      }
...
class Frame {
   ...
   public function new(index:Int, ?frameBitmapData:BitmapData, ?nineSlices:NineSliceSlices, ?renderWidth:Int, ?renderHeight:Int, ?sprite:Aseprite,
         ?frame:ase.Frame) {
      if (sprite != null)
         this.sprite = sprite;
      ...
      if (nineSlices != null) {
         ...
         for (chunk in frame.chunks.filter(c -> {
            c.header.type == ChunkType.CEL;
         })) {
            var cel:CelChunk = cast chunk;
            this.layers[cel.layerIndex].cel = (cel.celType == CelType.Linked ? sprite.aFrames[cel.linkedFrame].layers[cel.layerIndex].cel : new Cel(sprite,
               cel));
         }
      }

      for (layer in this.layers.filter(l -> {
         return (l.cel != null && (l.layerChunk.flags & LayerFlags.Visible != 0));
      })) {
